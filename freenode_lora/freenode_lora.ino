 ...
 *   - FN_FLAG_RELAYED: пакет не ретранслируется дважды
...
// ═══════════════════════════════════════════════════════════════
// Протокол FreeNode v0.3 (inline для Arduino .ino)
// ═══════════════════════════════════════════════════════════════
...
const char* transportName(TransportID t) { return t == TRANSPORT_ESPNOW ? "ESP-NOW" : "LoRa"; }
const char* transportIcon(TransportID t) { return t == TRANSPORT_ESPNOW ? "W" : "L"; }
...
void relayPacket(FNPacket& pkt, TransportID sourceTransport) {
  ...
  if (sourceTransport != TRANSPORT_ESPNOW) {
    if (sendViaEspNow(relay)) { relayed = true; Serial.printf("[RELAY] id=%u LoRa→ESP-NOW\n", pkt.id); }
  }
  if (sourceTransport != TRANSPORT_LORA) {
    if (sendViaLora(relay))   { relayed = true; Serial.printf("[RELAY] id=%u ESP-NOW→LoRa\n", pkt.id); }
  }
...
void oledRefresh() {
  ...
  display.printf("TX:%s R:%lu D:%lu", transportName(chooseBestTransport()), relayCount, dropCount);
...
void handleRxPacket(FNPacket& pkt, TransportID via, float rssi=0, float snr=0) {
  ...
  switch (pkt.type) {
    case FN_TYPE_TEXT: {
      char text[201];
      memcpy(text, pkt.payload, pkt.payloadLen); text[pkt.payloadLen] = '\0';
      Serial.printf("[RX %s] <%s> \"%s\"", transportName(via), senderStr, text);
      if (via == TRANSPORT_LORA) Serial.printf(" RSSI:%.0f SNR:%.1f", rssi, snr);
      Serial.println();
      oledAddLine(String("[")+transportIcon(via)+"]"+senderStr+"> "+String(text));
    ...
    case FN_TYPE_PING: {
      ...
      FNPacket pong; buildFNPacket(pong, FN_TYPE_PONG, DEFAULT_TTL, NULL, 0);
      markSeen(pong.id); sendViaBest(pong);
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (input == "/ping") {
      ...
      markSeen(pkt.id); sendViaBest(pkt);
      oledAddLine(String(">PING via ")+transportName(chooseBestTransport()));
    } else if (input == "/status") {
      Serial.printf("ID:%s E-TX:%lu E-RX:%lu L-TX:%lu L-RX:%lu Relay:%lu Drop:%lu\n",
        myMacStr, txCountEspNow, rxCountEspNow, txCountLora, rxCountLora, relayCount, dropCount);
    } else if (input == "/lora") { forceLoRa = true; oledAddLine("! Forced LoRa"); }
    else if (input == "/auto") { forceLoRa = false; oledAddLine("! Auto transport"); }
    else if (input.length() > 0) {
      FNPacket pkt;
      uint8_t pl[200]; uint8_t plen = min((int)input.length(), 200);
