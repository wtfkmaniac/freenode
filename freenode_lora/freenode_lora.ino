 ...
 *
 * TRIPLE TRANSPORT: ESP-NOW + LoRa SX1262 + BLE (NimBLE advertising)
 *
 * ИЗМЕНЕНИЯ v0.5 (на базе v0.4):
 *   - BLE advertising transport через NimBLE
 *   - PING/PONG/ANNOUNCE отправляются по всем 3 транспортам
 *   - Текст дублируется по BLE если payload ≤ BLE_MAX_PAYLOAD
 *   - BLE RX обновляет peer table
 *   - /status показывает BLE TX/RX
 *   - OLED: [B] = BLE
 *
 * Сохранено из v0.4:
 *   - Peer Table (MAC, transport, RSSI, lastSeen)
 *   - Unicast PING: /ping AA:BB
 *   - FN_FLAG_PONG_SENT для exactly-one PONG
 *   - ANNOUNCE пакеты
 *   - Cross-transport relay
 *   - Seen IDs дедупликация (128-slot ring buffer)
 *
 * Библиотеки:
 *   1. RadioLib by Jan Gromes
 *   2. Adafruit SSD1306
 *   3. Adafruit GFX Library
 *   4. NimBLE-Arduino by h2zero  ← NEW
 *
 ...
 *   Upload Speed: 115200
 *   Flash Size: 4MB
 *   Partition Scheme: "Default 4MB with spiffs"  ← ВАЖНО для NimBLE
 *
 * ВАЖНО: Подключи LoRa антенну перед включением!
 */
...
// ═══════════════════════════════════════════════════════════════
// Протокол FreeNode v0.4 (совместим с v0.3 freenode_mesh)
// ═══════════════════════════════════════════════════════════════
...
#define FN_TYPE_ANNOUNCE  0x06

#define FN_FLAG_RELAYED    0x01
#define FN_FLAG_UNICAST    0x02
#define FN_FLAG_PONG_SENT  0x04
...
#define BLE_ADV_HOLD_MS       600
#define BLE_MAX_DATA          25   // max manufacturer data payload
...
NimBLEAdvertising* pBLEAdv = nullptr;
uint32_t bleAdvExpireMs = 0;
bool bleReady = false;
...
public:
  void onResult(const NimBLEAdvertisedDevice* device) override {
    ...
    pkt.magic = d[0]; pkt.version = d[1];
    memcpy(pkt.src, d+2, 6); memcpy(pkt.dst, d+8, 6);
    pkt.ttl = d[14]; memcpy(&pkt.id, d+15, 2);
    pkt.type = d[17]; pkt.flags = d[18]; pkt.payloadLen = d[19];
    if (pkt.payloadLen > 200) return;
    if (data.length() < (size_t)(FN_HEADER_SIZE + pkt.payloadLen)) return;
    if (pkt.payloadLen > 0) memcpy(pkt.payload, d + FN_HEADER_SIZE, pkt.payloadLen);
    bleRxHead = next;
  }
};

static FNBLEScanCallbacks fnBLEScanCb;
...
void peerUpdate(const uint8_t* mac, TransportID via, int16_t rssi) {
  ...
  if (!p) return;
  p->lastSeen = millis(); p->transport = via; p->rssi = rssi; p->rxCount++;
}
void peerCleanup() {
  uint32_t now = millis();
  for (int i = 0; i < MAX_PEERS; i++) {
    if (peers[i].active && (now - peers[i].lastSeen > PEER_TIMEOUT_MS)) {
      ...
      peers[i].active = false; peerCount--;
    }
  }
}
void peerPrint() {
  ...
  for (int i = 0; i < MAX_PEERS; i++) {
    ...
    const char* tn = peers[i].transport == TRANSPORT_ESPNOW ? "ESP-NOW" :
                     peers[i].transport == TRANSPORT_LORA ? "LoRa" : "BLE";
    Serial.printf("  %-6s  %-8s  %-5d  %4lu sec  %lu\n",
      peers[i].id, tn, peers[i].rssi, age, peers[i].rxCount);
  }
...
void markSeen(uint16_t id) {
  seenIds[seenIdx] = id; seenIdx = (seenIdx + 1) % MAX_SEEN;
}
...
uint8_t myMac[6], myMacShort[2];
char myMacStr[6];
uint8_t broadcastAddr[] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};

uint32_t lastEspNowRx = 0;
uint32_t txCountEspNow=0, txCountLora=0, txCountBle=0;
uint32_t rxCountEspNow=0, rxCountLora=0, rxCountBle=0;
...
// Утилиты
// ═══════════════════════════════════════════════════════════════
void macShortToStr(uint8_t b1, uint8_t b2, char* out) { sprintf(out, "%02X:%02X", b1, b2); }
bool isMyMac(const uint8_t* mac) { return memcmp(mac, myMac, 6) == 0; }
bool isBroadcast(const uint8_t* mac) {
  for (int i=0;i<6;i++) if (mac[i]!=0xFF) return false; return true;
}
TransportID chooseBestTransport() {
  if (!forceLoRa && (millis()-lastEspNowRx < ESPNOW_PEER_TIMEOUT_MS)) return TRANSPORT_ESPNOW;
  return TRANSPORT_LORA;
}
const char* tName(TransportID t) {
  switch(t) { case TRANSPORT_ESPNOW: return "ESP-NOW"; case TRANSPORT_LORA: return "LoRa";
              case TRANSPORT_BLE: return "BLE"; default: return "?"; }
}
const char* tIcon(TransportID t) {
  switch(t) { case TRANSPORT_ESPNOW: return "W"; case TRANSPORT_LORA: return "L";
              case TRANSPORT_BLE: return "B"; default: return "?"; }
}
...
bool sendViaBle(FNPacket& pkt) {
  if (!bleReady) return false;
  if (pkt.payloadLen > BLE_MAX_PAYLOAD) return false;

  uint8_t buf[BLE_MAX_DATA];
  uint8_t pos = 0;
  buf[pos++] = pkt.magic; buf[pos++] = pkt.version;
  memcpy(buf+pos, pkt.src, 6); pos+=6;
  memcpy(buf+pos, pkt.dst, 6); pos+=6;
  buf[pos++] = pkt.ttl;
  memcpy(buf+pos, &pkt.id, 2); pos+=2;
  buf[pos++] = pkt.type; buf[pos++] = pkt.flags; buf[pos++] = pkt.payloadLen;
  if (pkt.payloadLen > 0) { memcpy(buf+pos, pkt.payload, pkt.payloadLen); pos+=pkt.payloadLen; }
  ...
  std::string mf;
  mf += (char)(BLE_COMPANY_ID & 0xFF);
  mf += (char)((BLE_COMPANY_ID >> 8) & 0xFF);
  mf += std::string((char*)buf, pos);
  advData.setManufacturerData(mf);
  pBLEAdv->setAdvertisementData(advData);
  pBLEAdv->start();
  bleAdvExpireMs = millis() + BLE_ADV_HOLD_MS;
  txCountBle++;
  return true;
}

bool sendViaBest(FNPacket& pkt) {
  return (chooseBestTransport()==TRANSPORT_ESPNOW) ? sendViaEspNow(pkt) : sendViaLora(pkt);
}
bool sendViaBestToPeer(FNPacket& pkt, const uint8_t* mac) {
  return (bestTransportToPeer(mac)==TRANSPORT_ESPNOW) ? sendViaEspNow(pkt) : sendViaLora(pkt);
}
// Отправить по всем транспортам (для PING, ANNOUNCE)
void sendViaAll(FNPacket& pkt) {
  sendViaEspNow(pkt);
  sendViaLora(pkt);
  sendViaBle(pkt);  // тихо пропустит если payload не влезает
}

// Cross-transport relay
void relayPacket(FNPacket& pkt, TransportID srcTransport) {
  if (pkt.ttl <= 1) return;
  FNPacket relay = pkt; relay.ttl--; relay.flags |= FN_FLAG_RELAYED;
  bool relayed = false;
  if (srcTransport != TRANSPORT_ESPNOW) if (sendViaEspNow(relay)) relayed = true;
  if (srcTransport != TRANSPORT_LORA)   if (sendViaLora(relay))   relayed = true;
  if (srcTransport != TRANSPORT_BLE)    if (sendViaBle(relay))    relayed = true;
  if (relayed) { relayCount++; Serial.printf("[RELAY] id=%u ttl=%d\n", pkt.id, relay.ttl); }
}
...
void oledRefresh() {
  ...
  display.printf("FN v0.5 [%s] P:%d", myMacStr, peerCount);
  display.setCursor(0,10);
  display.printf("TX:%s R:%lu D:%lu", tName(chooseBestTransport()), relayCount, dropCount);
...
void oledShowStatus(String l1, String l2="") {
  ...
  display.setCursor(0,0); display.println("FreeNode v0.5");
...
void espNowRxCallback(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  if (len>0 && len<=250 && !espNowRxReady) {
    memcpy((void*)espNowRxBuf, data, len); espNowRxLen = len; espNowRxReady = true;
  }
}
...
void sendAnnounce() {
  FNPacket pkt;
  buildFNPacket(pkt, FN_TYPE_ANNOUNCE, DEFAULT_TTL, (uint8_t*)myMacStr, strlen(myMacStr));
  markSeen(pkt.id);
  sendViaAll(pkt);
  Serial.printf("[ANNOUNCE] Sent ID:%s (all transports)\n", myMacStr);
}

// ═══════════════════════════════════════════════════════════════
// Обработка входящего пакета
// ═══════════════════════════════════════════════════════════════
void handleRxPacket(FNPacket& pkt, TransportID via, float rssi=0, float snr=0) {
  ...
  if (!isForMe) {
    if (!alreadySeen(pkt.id) && pkt.ttl > 1) {
      markSeen(pkt.id); relayPacket(pkt, via);
    }
    return;
  }
  ...
  else if (via == TRANSPORT_BLE) rxCountBle++;
  ...
  switch (pkt.type) {
    case FN_TYPE_TEXT: {
      char text[201]; memcpy(text, pkt.payload, pkt.payloadLen); text[pkt.payloadLen]='\0';
    ...
    case FN_TYPE_PING: {
      ...
      Serial.printf("[RX %s] PING from %s (id=%u %s)\n",
        tName(via), senderStr, pkt.id, (pkt.flags&FN_FLAG_UNICAST)?"unicast":"broadcast");
      oledAddLine(String("[")+tIcon(via)+"]"+senderStr+"> PING");

      FNPacket pong;
      buildFNPacket(pong, FN_TYPE_PONG, DEFAULT_TTL, NULL, 0);
      memcpy(pong.dst, pkt.src, 6); pong.flags = FN_FLAG_UNICAST;
      memcpy(pong.payload, &pkt.id, 2); pong.payloadLen = 2;
      markSeen(pong.id);
      sendViaBestToPeer(pong, pkt.src);
      sendViaBle(pong);  // дублируем PONG по BLE

      if (isBroadcast(pkt.dst)) {
        FNPacket flag = pkt; flag.flags |= FN_FLAG_PONG_SENT; flag.ttl = 2;
        sendViaEspNow(flag); sendViaLora(flag);
      }
    ...
    case FN_TYPE_HEARTBEAT: break;
    default:
      Serial.printf("[RX] Unknown type 0x%02X from %s\n", pkt.type, senderStr);
  }

  if (pkt.ttl > 1 && pkt.type != FN_TYPE_HEARTBEAT && isBroadcast(pkt.dst))
    relayPacket(pkt, via);
}
...
void setup() {
  ...
  Serial.println("Transports: ESP-NOW + LoRa SX1262 + BLE");
  ...
  oledShowStatus("FreeNode v0.5", String("ID: ")+myMacStr);

  // ESP-NOW
  esp_wifi_set_channel(ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE); delay(50);
  if (esp_now_init() == ESP_OK) {
    esp_now_peer_info_t p={}; memcpy(p.peer_addr, broadcastAddr, 6);
    p.channel=ESPNOW_CHANNEL; p.encrypt=false; esp_now_add_peer(&p);
  ...
  radio.setCurrentLimit(60.0); radio.setDio2AsRfSwitch(true); radio.setCRC(true);
  radio.setPacketReceivedAction(loraRxCallback); radio.startReceive();
  Serial.println("[LoRa] OK");

  // BLE (NimBLE)
  Serial.print("[BLE] Init NimBLE... ");
  NimBLEDevice::init("FreeNode");
  NimBLEDevice::setPower(ESP_PWR_LVL_P3);
  pBLEScan = NimBLEDevice::getScan();
  pBLEScan->setScanCallbacks(&fnBLEScanCb, true);
  ...
  uint16_t advU = BLE_ADV_INTERVAL_MS*1000/625;
  pBLEAdv->setMinInterval(advU); pBLEAdv->setMaxInterval(advU);
  bleReady = true;

  Serial.println();
  Serial.printf("[LoRa] Freq:%.1fMHz SF%d BW%.0fkHz\n", LORA_FREQ, LORA_SF, LORA_BW);
  Serial.printf("[ESP-NOW] CH:%d\n", ESPNOW_CHANNEL);
  Serial.printf("[BLE] Adv:%dms MaxPayload:%d\n", BLE_ADV_INTERVAL_MS, BLE_MAX_PAYLOAD);
  Serial.println();
  Serial.println("Commands:");
  Serial.println("  /ping          — broadcast ping (all transports)");
  Serial.println("  /ping AA:BB    — unicast ping");
  Serial.println("  /peers         — peer table");
  Serial.println("  /status        — statistics");
  Serial.println("  /lora /auto    — transport selection");
...
void loop() {
  ...
  if (loraRxFlag) {
    ...
    } else if (state == RADIOLIB_ERR_CRC_MISMATCH) Serial.println("[LoRa] CRC err");
    radio.startReceive();
  }
  ...
  while (bleRxHead != bleRxTail) {
    FNPacket& pkt = bleRxBuf[bleRxTail];
    bleRxTail = (bleRxTail + 1) % BLE_RX_BUF_SIZE;
    handleRxPacket(pkt, TRANSPORT_BLE);
  }

  // ── BLE advertising lifecycle ──
  if (bleAdvExpireMs && millis() > bleAdvExpireMs) {
    pBLEAdv->stop(); bleAdvExpireMs = 0;
  ...
  if (Serial.available()) {
    ...
    if (input.length() == 0) {
      ...
      FNPacket pkt; buildFNPacket(pkt, FN_TYPE_PING, DEFAULT_TTL, NULL, 0);
      markSeen(pkt.id); sendViaAll(pkt);
      Serial.printf("[CMD] Broadcast PING id=%u (all transports)\n", pkt.id);
      oledAddLine(">PING (all)");
    } else if (input.startsWith("/ping ")) {
      String target = input.substring(6); target.trim();
      PeerEntry* peer = nullptr;
      for (int i=0;i<MAX_PEERS;i++)
        if (peers[i].active && String(peers[i].id)==target) { peer=&peers[i]; break; }
      if (!peer) {
        Serial.printf("[PING] Peer %s not found\n", target.c_str());
      } else {
        FNPacket pkt; buildFNPacket(pkt, FN_TYPE_PING, DEFAULT_TTL, NULL, 0);
        memcpy(pkt.dst, peer->mac, 6); pkt.flags = FN_FLAG_UNICAST;
        markSeen(pkt.id);
        sendViaBestToPeer(pkt, peer->mac);
        sendViaBle(pkt);  // дублируем по BLE
        Serial.printf("[CMD] Unicast PING → %s\n", target.c_str());
      ...
      Serial.printf("  ESP-NOW TX/RX: %lu/%lu\n", txCountEspNow, rxCountEspNow);
      Serial.printf("  LoRa    TX/RX: %lu/%lu\n", txCountLora, rxCountLora);
      Serial.printf("  BLE     TX/RX: %lu/%lu\n", txCountBle, rxCountBle);
      ...
      if (plen <= BLE_MAX_PAYLOAD) sendViaBle(pkt);
      oledAddLine(String("[WLB]>")+input.substring(0,14));
      Serial.printf("[TX] \"%s\"\n", input.c_str());
    }
  }

  // ── Кнопка = broadcast PING ──
  ...
  if (millis()-lastOled > 2000) { lastOled=millis(); oledRefresh(); }

  // ── Peer cleanup ──
  static uint32_t lastCleanup = 0;
  if (millis()-lastCleanup > 10000) { lastCleanup=millis(); peerCleanup(); }

  // ── ANNOUNCE каждые 30 сек ──
  static uint32_t lastAnnounce = 0;
  if (millis()-lastAnnounce > 30000) { lastAnnounce=millis(); sendAnnounce(); }

  // ── ESP-NOW heartbeat ──
  static uint32_t lastHB = 0;
  if (!forceLoRa && millis()-lastHB > 5000) {
    lastHB = millis();
    FNPacket hb; buildFNPacket(hb, FN_TYPE_HEARTBEAT, 1, NULL, 0);
    markSeen(hb.id); sendViaEspNow(hb);
    sendViaBle(hb);  // BLE heartbeat тоже
  }
